...
def test():
        ...
        # 解包最优参数并转换为绝对时间
        v_opt = v_drone
        theta_opt = theta_drone
        
        # 计算真实的投放时间点
        t_drop1 = t_drone_fly
        t_delta_drop2 = t_delta_drop2
        t_delta_drop3 = t_delta_drop3
        
        t_drop2 = t_drop1 + t_delta_drop2
        t_drop3 = t_drop2 + t_delta_drop3
        
        t_drop_opt = np.array([t_drop1, t_drop2, t_drop3], dtype=float)
        t_delay_opt = np.array([t_decoy_delay_1, t_decoy_delay_2, t_decoy_delay_3], dtype=float)
        t_expl_opt = t_drop_opt + t_delay_opt
